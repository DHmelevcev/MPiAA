...
int main(int argc, char* argv[])
{
	...
	for (auto i : sizes)
	{
		Graph graph(generate_complete_graph_edges(i, log(i)));
		...
		pair<vector<int>, double> path2 = tsp_lp(graph, start_vertex);
		t2 = std::chrono::high_resolution_clock::now();
		double time2 = std::chrono::duration<double>(t2 - t1).count();
		cout << "\ntsp_lp:     lenght: " << setw(6) << path2.second << "\ttime: " << setw(8) << time2;

		if (graph.get_vertices().size() < 19)
		{
			t1 = std::chrono::high_resolution_clock::now();
